...
const isValid = function(phrase) {
	const shallow = [].concat(phrase);
    while (shallow.length > 0) {
        const word = shallow.pop();
        if (shallow.indexOf(word) > -1) return false;
    }
    return true;
};
...
// create a hash sum by adding
// charcodes from the word together
// https://stackoverflow.com/questions/194846/is-there-any-kind-of-hash-code-function-in-javascript#8076436
const simplewordHash = function(word){
	word = sortWord(word); // sort to create same hashes for all annagrams
    let hash = 0;
...
const correctPhrases = function(list, useHashes) {
    let sum=0;
    for (let passPhrase of list) {
    	if (useHashes && !isValid(passPhrase.map(simplewordHash))) continue;
    	if (!isValid(passPhrase)) continue;
        sum ++;
    }
    return sum;
}

const test1= ["aa bb cc dd ee"].map(toArray);
const test2= ["aa bb cc dd aa"].map(toArray);
const test3= ["aa bb cc dd aaa"].map(toArray);
console.log(correctPhrases(test1));
console.log(correctPhrases(test2));
console.log(correctPhrases(test3));
console.log(correctPhrases(input));

/*
--- Part Two ---

For added security, yet another system policy has been put in place. Now, a valid passphrase must contain no two words
that are anagrams of each other - that is, a passphrase is invalid if any word's letters can be rearranged
...
const test4= ["abcde fghij"].map(toArray);
const test5= ["abcde xyz ecdab"].map(toArray);
const test6= ["a ab abc abd abf abj"].map(toArray);
const test7= ["iiii oiii ooii oooi oooo"].map(toArray);
const test8= ["oiii ioii iioi iiio"].map(toArray);
const test9= ["hey hye"].map(toArray);
const test10= ["af be"].map(toArray);



console.log(1, correctPhrases(test4, true));
console.log(0, correctPhrases(test5, true));
console.log(1, correctPhrases(test6, true));
console.log(1, correctPhrases(test7, true));
console.log(0, correctPhrases(test8, true));
console.log(0, correctPhrases(test9, true));
console.log(1, correctPhrases(test10, true));

console.log(correctPhrases(input, true));
