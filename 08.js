...
const compare = function(a, str, b) {
	//console.log(a, str, b);
    if (str === ">") return a > b;
    if (str === ">=") return a >= b;
    if (str === "<") return a < b;
    ...
    if (str === "==") return a === b;
    if (str === "!=") return a !== b;
    throw new Error("unkown compare " + str);
}

const getValue = function(instr, num) {
    const parsed = parseInt(num);
	//console.log(instr, parsed)
    if (instr === "inc") return parsed;
    if (instr === "dec") return -parsed;
...
const getMap = function(list) {
	const reg = {};
	let largest = -10000000;
    for (let instruction of list) {
        if (instruction[3] !== "if"){
			console.log(instruction);
			throw new Error("expected if, got " + instruction[3]);
		}
            
        const compareTarget = reg[instruction[4]] || 0;
        const compareValue = parseInt(instruction[6]);
        if (!compare(compareTarget, instruction[5], compareValue))
			continue;
		
        if (!reg[instruction[0]]) 
			reg[instruction[0]] = 0; 
			
		reg[instruction[0]] += getValue(instruction[1], instruction[2]);
		if (reg[instruction[0]] > largest)
			largest = reg[instruction[0]];
	}
	return {largest:largest, map:reg};
};

const getLargest = function(arr) {
	const map = getMap(arr).map;
	console.log(map);
...
l dec 367 if uix >= -3376`).split("\n").map(toArray);

console.log(getLargest(input));
...
console.log(getMap(test1).largest)
console.log(getMap(input).largest)
