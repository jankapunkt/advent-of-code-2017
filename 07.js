...
const refsToArray = function(refs) {
    return refs.replace(/\s+/g, "").split(","); // "ktlj, cntj, xhth" to ["ktlj","cntj","xhth"]
};

const weightToNum = function(weight) {
	return parseInt(weight.replace("(", "").replace(")", ""));
}
...
const parseline = function(line) {
    ...
    const splitRef = line.split("->");

    // refs on the right
    if (splitRef[1]) {
        obj.refs = refsToArray(splitRef[1]);
    }
    
    // name and weight on the left
    const nameWeight = splitRef[0].split(" ");
    obj.name = nameWeight[0];
    obj.weight = weightToNum(nameWeight[1]);
	...
	for (let entry of arr) {
    	if (entry.refs) {     
            for (let ref of entry.refs) {
				map[ref] = {name: entry.name, weight: entry.weight};
            }
        }
    }
    return map;
...
const getBottom = function(arr) {
	const map = getMap(arr);
    // resolve bottom ref
	const keys = Object.keys(map);
    for (let key of keys) {
    	const child = map[key].name;
    	
        // if there is no entry for
        // the child it must be the bottom
...
const removeNoRefs = function(element) {
	return !!element.refs;
}
...
twxghfp (91)
tngvv (27)
koklf (61) -> jqtbsy, eetpan, vamssbi, rvwvokn, vpjsxth`).split("\n").map(parseline);

console.log(getBottom(input));
...
const getWeightMap = function(list) {
	const map = {};
    for (let entry of list) {
    	map[entry.name] = entry.weight;
    }
    return map;
}

const getWeight = function(entry, weightMap, list) {
	let sum = entry.weight;
    let refs = entry.refs || [];
    for (let ref of refs) {
        const refElement = getEntry(list, ref);
        if (!refElement || !refElement.refs) {
	        sum += weightMap[ref]; //this is the top
        }else{
        	sum+= getWeight(refElement, weightMap, list)
        }
    }
    return sum;
};

const getEntry = function(list, name) {
	for (let entry of list) {
...
const weightBranch = function(list) {
	const weightMap = getWeightMap(list);
    const refMap = getMap(list);
	const bottom = getEntry(list, getBottom(list).name);

	const first =  getEntry(list, bottom.refs[0]);
	const second =  getEntry(list, bottom.refs[1]);
    const third =  getEntry(list, bottom.refs[2]);
	console.log(getWeight(first, weightMap, list));
	console.log(getWeight(second, weightMap, list));
    console.log(getWeight(third, weightMap, list));
}

console.log(weightBranch(input));

https://jsfiddle.net/qp1Lf4h6/3/
